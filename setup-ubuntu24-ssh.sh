...
# Ubuntu 24.04 Docker Container Setup Script for Password-less SSH Access
# This script creates an Ubuntu 24.04 container with SSH daemon configured for key-based authentication

set -e

CONTAINER_NAME="ubuntu24-ssh"
SSH_PORT="2223"
IMAGE_NAME="ubuntu24-ssh"
SSH_KEY_NAME="ubuntu24_key"

echo "=== Ubuntu 24.04 Docker Container SSH Setup ==="
...
generate_ssh_key() {
    if [ ! -f "$SSH_KEY_NAME" ]; then
        ...
        ssh-keygen -t rsa -b 4096 -f "$SSH_KEY_NAME" -N "" -C "ubuntu24-container-access"
...
build_image() {
    ...
    docker build -t $IMAGE_NAME -f Dockerfile.ubuntu24 .
...
cleanup_known_hosts() {
    ...
    # Remove any existing entries for localhost:2223 from known_hosts
...
test_ssh_connection() {
    ...
    if ssh -i "$SSH_KEY_NAME" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=10 -p $SSH_PORT ubuntu@localhost "echo 'SSH connection successful!'" 2>/dev/null; then
        echo "✓ SSH connection test passed!"
    else
        echo "⚠ SSH connection test failed. You may need to wait a moment for the SSH daemon to fully start."
        echo "Try connecting manually with: ssh -i $SSH_KEY_NAME -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p $SSH_PORT ubuntu@localhost"
    fi
}
...
show_connection_info() {
    ...
    echo "To connect to the Ubuntu 24.04 container:"
    echo "  ssh -i $SSH_KEY_NAME -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p $SSH_PORT ubuntu@localhost"
    echo
...
main() {
    case "${1:-start}" in
        "start")
            echo "Starting Ubuntu 24.04 container setup..."
            cleanup
            cleanup_known_hosts
            generate_ssh_key
            build_image
            ...
            ;;
        "stop")
            echo "Stopping Ubuntu 24.04 container..."
            docker stop $CONTAINER_NAME
            echo "Container stopped: $CONTAINER_NAME"
            ;;
        "clean")
            echo "Cleaning up Ubuntu 24.04 container and image..."
            cleanup
            echo "Cleanup completed"
            ;;
        "connect")
            echo "Connecting to Ubuntu 24.04 container..."
            ssh -i "$SSH_KEY_NAME" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p $SSH_PORT ubuntu@localhost
            ;;
        "info")
            show_connection_info
            ;;
        *)
            ...
            echo "  start  - Build and start the Ubuntu 24.04 container with SSH (default)"
